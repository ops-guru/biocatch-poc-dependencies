...
start_crossplane_pod = KubernetesPodOperator(
    ...
    image="alpine/helm",
    cmds=['/bin/sh', '-c', 'helm repo add poc https://raw.githubusercontent.com/ops-guru/biocatch-poc-dependencies/main/helm/packages/ && helm repo update && helm upgrade -i {{ var.value.crossplane_app_name }}-crs poc/crossplane-service-account --atomic --debug -f https://raw.githubusercontent.com/ops-guru/biocatch-poc-dependencies/main/airflow/values-crossplane.yaml --set-string fullnameOverride={{ var.value.crossplane_app_name }} --set-string workloadIdentity.project=artur-bolt-development --set-string workloadIdentity.namespace={{ var.value.crossplane_app_namespace }}--set-string workloadIdentity.serviceAccount={{ var.value.crossplane_app_name }} '],
