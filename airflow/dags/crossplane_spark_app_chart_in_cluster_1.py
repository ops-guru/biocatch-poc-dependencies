...
start_spark_pod = KubernetesPodOperator(
    ...
    namespace=crossplane_app_namespace,
    image="alpine/helm",
    cmds=['/bin/sh', '-c', 'helm repo add poc https://raw.githubusercontent.com/ops-guru/biocatch-poc-dependencies/main/helm/packages/ && helm repo update && helm upgrade -i {{ var.value.crossplane_app_name }}-spark poc/spark-application --atomic --debug -f https://raw.githubusercontent.com/ops-guru/biocatch-poc-dependencies/main/airflow/values-spark-app.yaml --set-string name={{ var.value.crossplane_app_name }} --set-string driver.serviceAccount={{ var.value.crossplane_app_name }} --set-string executor.serviceAccount={{ var.value.crossplane_app_name }} --set-string fullnameOverride={{ var.value.crossplane_app_name }} --set-string serviceAccount.annotations[0].iam.gke.io/gcp-service-account={{ var.value.crossplane_app_name }}@artur-bolt-development.iam.gserviceaccount.com'],
    service_account_name="helm-identity",
    do_xcom_push=False,
    is_delete_operator_pod=True,
    get_logs=True,
